...
<body>
	...
	<div class="page-content">
		...
		<blockquote class="layui-elem-quote"
			...
			<div class="input-card"
				style="left: 100px; height: 340px; width: 400px; background: #333; opacity: .80; position: fixed; left: 75%; top: 10%;">

				<span class="x-red"><h3>接单成功！</h3></span> <img
					src="images/success.jpg"
					style="height: 30px; width: 30px; position: relative; left: 120px; top: -25px;">

				<div style="color: white;">司机已经接到订单</div>
				...
				<input type="submit"
					...
					class="layui-btn" value="点击获取订单详情"
					style="display: block; mergin: 0 auto;"> <input
				...
				<form action="torun">
					...
					<view class="btn1"> <input type="button"
						class="btn btn-primary btn-lg" onclick="refresh()"
						style="display: block; mergin: 0 auto; width: 130px; margin-top: 20px;margin-left:30px;"
						value="获取司机位置"> </view>
					<view class="btn1"> <input type="submit"
						class="btn btn-primary btn-lg" onclick="get()"
						style="display: block; mergin: 0 auto; width: 130px; margin-top: 20px;margin-left:50px;"
						value="确认上车"> </view>
				</form>
			</div>
		</blockquote>
	...
	<%=order.getSlat()%>
		;
		var lon =
	<%=order.getDriver().getLongitude()%>
		;
		var lat =
	<%=order.getDriver().getLatitude()%>
		;
		...
		// 根据起终点经纬度规划驾车导航路线
		driving.search(new AMap.LngLat(lon, lat), new AMap.LngLat(slng, slat),
	...
	<%System.out.println(order.getDriver().getLatitude());%>
		function get() {
			document.getElementById("orderid").value = document
					.getElementById("n1").innerHTML;
		}
		function refresh() {
			location.reload();
		}
				...
				<form action="deleteorder">
					<div class="modal-footer">
						<div class="modal-body"
							...
							请对司机打分:<span class="box"> <span>★</span> <span>★</span> <span>★</span>
								<span>★</span> <span>★</span>
							</span> <span id="2"><s:property value="order.estimatedtod" />0</span>分
						</div>
						<input type="hidden" id="order" name="order.orderID"> <input
							type="hidden" id="passenger" name="order.passenger.passengerID">
						<input type="hidden" id="driver" name="order.driver.driverID">
						<input type="hidden" id="1" name="order.estimateptod"
							readonly="readonly" value="0"/> <input type="submit"
							class="btn btn-primary" value="完成评价" onclick="f()">
					</div>
				</form>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function f() {
			document.getElementById("order").value = document
					.getElementById("n2").innerHTML;
			document.getElementById("passenger").value = document
					.getElementById("n3").innerHTML;
			document.getElementById("driver").value = document
					.getElementById("n4").innerHTML;
		}
